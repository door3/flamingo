 ...
 *
 * @ViewsField("hr_status_primary_review")
 */
class HrReviewPrimary extends FieldPluginBase {
  ...
  public function render(ResultRow $values) {
    ...
    $primary_manager = $this->entityTypeManager->getStorage('user')->load($primary_manager_id);

    $status = [
      '#type' => 'markup',
      '#markup' => $primary_manager->getAccountName() . ': <span class="">-</span>',
    ];
      ...
      if ($manager_review_result) {
        ...
        if ($review) {
          ...
          if ($review->isDraft()) {
            $status = [
              '#type' => 'markup',
              '#markup' => $primary_manager->getAccountName() . ': <span class="badge badge-warning">draft</span>',
            ];
          }
          else {
            $status = [
              '#type' => 'markup',
              '#markup' => $primary_manager->getAccountName() . ': <span class="badge badge-success">' . $review_data['pimary_manager_status_review'] . '</span>',
            ];
          }
        }
      }
